<div class="container">
		<div class="row-fluid">
        <h1><?php echo ($action == 'edit') ? 'Modificar ' : 'Nuevo ' ?>Tutor</h1>		
        	<form method="POST" action="tutor.php?action=<?php echo $action; ?>">
				<div class="col-lg-12">
					<br><br>
					<div class="row">
						<div class="col-lg-6">
    					...
    					<div class="mb-3">
        					...
        					<select name="data[id_usuario]" class="form-control" required>
                                ...
                                <select name="data[id_rol_tutor]" class="form-control" required>
            					<?php
            						foreach ($datarol_tutor as $key => $tut) :
                					$selected = "";
                					if ($tut['id_rol_tutor'] == $data[0]['id_rol_tutor']) :
                    					$selected = "selected";
                					endif;
            					?>
                				<option value="<?php echo $tut['id_rol_tutor']; ?>" <?php echo $selected; ?>><?php echo $tut['rol_tutor']; ?> </option>
            					<?php endforeach; ?>
      		  				</select>
		  					</div>
						</div>
                        <div class="mb-3">
        <?php if ($action == 'edit') : ?>
            <input type="hidden" name="data[id_tutor]" value="<?php echo isset($data[0]['id_tutor']) ? $data[0]['id_tutor'] : ''; ?>">
        <?php endif; ?>
